...
# Call the pack_soft_sphere function
device = "cuda" if torch.cuda.is_available() else "cpu"
dtype = torch.float32
comp = Composition("Cu50Zr50")
sigma: float = 2.5
scale_volume: float = 1.0
state = pack_soft_sphere(
    comp=comp, device=device, dtype=dtype, sigma=sigma, scale_volume=scale_volume
)
...
assert cell.shape == (3, 3), "cell should be a 3x3 matrix"
assert positions.shape == (100, 3)
assert masses.shape == (100,)
assert atomic_numbers.shape == (100,)
...
for step in range(N_steps):
    ...
    frames.append(
        {
            "cell": state.cell.detach().cpu().numpy().tolist(),
            "positions": state.positions.detach().cpu().numpy().tolist(),
...
result = {
    ...
    "forces": state.forces.detach().cpu().numpy().tolist(),
}
print(result)
